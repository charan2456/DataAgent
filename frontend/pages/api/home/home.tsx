...
import { LLM, AgentID, Agents, fallbackModelID } from '@/types/agent';
...
const Home = ({
  ...
  const handleNewConversation = async () => {
    const newConversation: Conversation = {
      ...
      messages: [],
      agent: {
        id: Agents[defaultAgentId].id,
        name: Agents[defaultAgentId].name,
        maxLength: Agents[defaultAgentId].maxLength,
        tokenLimit: Agents[defaultAgentId].tokenLimit,
        llm: Agents[defaultAgentId].llm,
      },
      prompt: DEFAULT_SYSTEM_PROMPT,
  ...
  return (
    ...
    >
      <Head>
        ...
        <meta name="description" content="Data Agent - AI-powered data analysis platform" />
        <meta
          name="viewport"
      ...
      {selectedConversation && (
        <main
          className={`flex h-screen w-screen flex-col text-sm text-white ${lightMode} font-[Montserrat] bg-gradient-to-br from-slate-50 to-slate-100`}
        >
          <Modal
            open={showTerms}
...
export const getServerSideProps: GetServerSideProps = async ({ locale }) => {
  const defaultAgentId =
    (process.env.DEFAULT_AGENT &&
      Object.values(AgentID).includes(
        process.env.DEFAULT_AGENT as AgentID,
      ) &&
      process.env.DEFAULT_AGENT) ||
    fallbackModelID;
