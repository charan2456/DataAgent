...
export const ChatMessage: FC<Props> = memo(
  ({ message, messageIndex, onEdit, onRegenerate, scrollToBottom }) => {
    ...
    const renderRichIntermediateSteps = () => {
      ...
      while (curIndex < message.richContent.intermediateSteps.length) {
        ...
        if (item.type == 'tool') {
          ...
          let curTool = allEnabledPlugins.find(
            (plugin) =>
    ...
    return (
      <div
        className={`w-full bg-transparent text-slate-800 relative flex items-end
      ...
      >
        ...
        <Paper
          ...
          } my-4 flex items-center p-3 text-base rounded-2xl shadow-soft
          ${
            message.role === 'user'
              ? 'bg-gradient-to-br from-primary-100 to-primary-50 mr-4 ml-40 border border-primary-200'
              : 'bg-white ml-4 mr-40 border border-slate-200 shadow-medium'
          }`}
        >
          {message.role === 'user' && (
            <div className="w-full">
              {isEditing && message.type !== 'rich_message' ? (
                <div className={`flex flex-col w-[${userMsgWidth}px]`}>
                  ...
                  <textarea
                    ref={textareaRef}
                    className={`min-w-[200px] resize-none whitespace-pre-wrap border-none bg-primary-50`}
        ...
        {message.role === 'user' && (
          <PersonIcon className="bg-gradient-to-br from-primary-600 to-primary-700 w-8 h-8 text-white p-1 rounded-full mr-28 mb-[13px] shadow-md" />
        )}
      </div>
    );
  },
